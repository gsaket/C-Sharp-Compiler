...
// For storing local vars on stack
vector<string> LocalScopeVars;
...
string getRegister(int instr, string var, string operand1 = "$$", string operand2 = "$$"){
	for(auto reg : Registers){
		if(getRegisterDescriptor(reg) == var){
			if(var == operand1 || var == operand2)
				continue;
			//cerr<<reg<<":: "getRegisterDescriptor(reg)<<endl;
			return reg;
		}
	 ...
	 *    }
	 *    //cerr<<reg_<<":: "getRegisterDescriptor(reg_)<<endl;
	 *    return reg_;
	 *}
	 */
...
void init()
{
	...
	keywords.insert("label");keywords.insert("param");keywords.insert("pop");
	keywords.insert("int");keywords.insert("char");keywords.insert("string");
	keywords.insert("bool");keywords.insert("NULL");keywords.insert("null");
...
string genx86(vector<string> instr){
	...
	else if(op=="&&")
	{
		...
		else
		{
			string lop2 = getLocation(operand2);
			string lop1 = getLocation(operand1);
			//cerr<<"destreg: "<<destreg<<" lop1: "<<lop1<<" lop2: "<<lop2<<endl;
			x86 += "movl "+lop1+", "+destreg+"\n";
	...
	else if(op == "ifgoto")
	{
		...
		string lop2 = getLocation(operand2);
		// Compare
		//cerr<<"IFGOT instr: "<<instr[2]<<" "<<instr[3]<<" "<<instr[4]<<" "<<instr[5]<<endl;
		//cerr<<"lop1: "<<lop1<<" lop2"<<lop2<<" destreg: "<<destreg<<endl;
		if(lop1 != operand1 && lop1[0]!='$'){
			x86+= "cmpl "+lop2+", "+lop1+"\n";
		}else{
			x86 += "cmpl "+lop2+", "+destreg+"\n";
	...
	{
		// line, call, funct, num_args, return_variable
		string  func,rvar,num_args;
		func = instr[2];
		num_args=instr[3];
		rvar = instr[4];
		// Spilling the registers
		...
		if(stringToInt(num_args) == 0){
			// store all local variables onto the stack
			for(int i=0;i<LocalScopeVars.size();i++){
				string lop=getLocation(LocalScopeVars[i]);
				x86+="pushl "+lop+"\n";
			}
		}
		x86 += "call "+func+"\n";
		int off=4*stringToInt(num_args);
		if(off != 0){
			x86+="addl $"+to_string(off)+", %esp\n";
		}
		// restore all local variables
		for(int i=(int)(LocalScopeVars.size())-1;i>=0;i--){
			x86 += "pop "+LocalScopeVars[i]+"\n";
		}
	...
	else if(op == "read")
	{
		...
		string fmt_string="read_int";
		string typ=instr[3];
		if(typ == "char")fmt_string="read_char";
		x86 += "pushl $"+var+"\n";
		x86 += "pushl $"+fmt_string+"\n";
		x86 += "call scanf\n";
	}
	//////////
	// exit
	...
	{
		// line, array, type, cnt, array_name
		string array_length = instr[3];
		string array_name = instr[4];
	...
	// label
		...
		string label=instr[2];
		x86 += label+": \n";
	}
	else if(op == "param"){
		// line, param, value, which_arg
		string var;
		var = instr[2];
		int which_arg=stringToInt(instr[3]);
		if(which_arg == 1){
			// store all local variables onto the stack
			for(int i=0;i<LocalScopeVars.size();i++){
				string lop=getLocation(LocalScopeVars[i]);
		...
		// line, pop, variable, offset
		int line_no=stringToInt(instr[0]);
		string var=instr[2];
		string off=instr[3];
		string destreg = getRegister(line_no, var);
		x86 += "movl "+off+"(%ebp), "+destreg+"\n";
		if(!isInteger(var))setAddressDescriptor(var, destreg);
		if(!isInteger(var))setRegisterDescriptor(destreg, var);
		// add var to LocalScopeVars
		LocalScopeVars.pb(var);
	}else if(op == "int"){
		// line, int, var
		string var=instr[2];
		// add var to LocalScopeVars
	...
	else if(op == "print")
	{
		...
		//print statement
		string fmt_string="write_int";
		string typ=instr[3];
		if(typ == "char")fmt_string="write_char";
...
int main(int argc, char** argv){
	...
	for(int i=0;i<NumInstr;i++)
	{
		if(instructions[i].size()>1){
			...
			if(instructions[i][1] == "array"){
				string aName= instructions[i][3];
				if(all_variables.find(aName)!=all_variables.end())
				all_variables.erase(aName); //#arjun
				aName= instructions[i][4];
				if(all_variables.find(aName)!=all_variables.end())
				all_variables.erase(aName); //#arjun
			}
			if(instructions[i][1] == "label"){
				string lname = instructions[i][2];
				all_variables.erase(lname);
			}
		}
	}
	...
	DataSection+="write_int:\n.ascii \"%d\\n\\0\"\n";
	DataSection+="read_int:\n.ascii \"%d\"\n";
	//DataSection+="write_char:\n.ascii \"%c\\n\\0\"\n";
	//DataSection+="read_char:\n.ascii \"%c\\n\"\n";
